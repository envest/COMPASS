    ...
    with loompy.connect(infile) as ds:
        ...
        for i in prefiltered_loci:
            ...
            if not use_whitelist:
                ...
                # Remove loci for which one allele has always less than x% of the reads, when the allele is detected (but still allow some sequencing errors)
                # They probably correspond to sequencing/mapping errors.
                threshold = 0.30
                #if nonsynonymous_variant: threshold = 0.28
                #if nonsynonymous_variant and n_amplicons>100: threshold=0.28
